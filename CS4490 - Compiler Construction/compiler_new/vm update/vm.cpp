...
int assemblerPass2(ifstream &assemblyFile, int& instrCounter, vector<int32_t>& reg, vector<unsigned char> &MEM, map<string, int32_t> &ST, int &beginningAddress)
{
	...
	while (!assemblyFile.eof())
	{
		...
		else if (anInstruction(tokens[0]))
		{
			...
			if (tokens[2].size() > 0)
			{
				bool atEnd = it == ST.end();
				bool aReg = !aRegister(tokens[2]);
				bool aNum = !aNumber(tokens[2]);
				if (tokens[2].at(0) == ';'){}
				else if (atEnd && aReg && aNum)
				{
		...
		else
		{
			...
			else if (anInstruction(tokens[1]))
			{
				...
				if (tokens[3].size() > 0)
				{
					...
					bool atEnd = it == ST.end();
					bool aReg = !aRegister(tokens[3]);
					bool aNum = !aNumber(tokens[3]);
					if (tokens[3].at(0) == ';'){}
					else if (atEnd && aReg && aNum)
					{
						cout << "Semantics Error on Line " << instrCounter << ": " << tokens[1] << " Instruction - Label not in Table.\n";
	...
	while (running)
	{
		...
		for (int instrNumber = 0; instrNumber < instructionsRun; instrNumber++)
		{
			if (threads[currentTID] >= 0)
			{
				...
				std::string worthlessStatement = "pokemon";
				...
				case 1:
					if (reg[instrSet->opd2] < 0 || reg[instrSet->opd2] > MEM.size())
						throw std::runtime_error("VM Broke - Bad Index.");
...
bool aNumber(string& s)
{
	...
	if (ts.size() > 0 && (ts.at(0) == '+' || ts.at(0) == '-'))
		ts = ts.substr(1);

	if (ts.size() > 0 && (ts.at(0) == '0' || ts.at(0) == '1' || ts.at(0) == '2' || ts.at(0) == '3' || ts.at(0) == '4' || ts.at(0) == '5' || ts.at(0) == '6'
		|| ts.at(0) == '7' || ts.at(0) == '8' || ts.at(0) == '9'))
		return !ts.empty() && ts.find_first_not_of("0123456789") == std::string::npos;
	else return false;
}
int registerNumber(string registerName)
{
	...
	else
	{
		...
		//remove pesky #
		if (registerName.size() > 1 && registerName.at(0) == '#')
			registerName = registerName.substr(0, registerName.size() - 1);
