...
void TCGenerator::writePush(quad* quadStatement)
{
	...
	std::string usedAsRef = st->symbolTable[quadStatement->op1]->getRefFlag();
	if (type == "int" || type == "ref" || usedAsRef == "Y")
	{
...
void TCGenerator::writeRef(quad* quadStatement)
{
	...
	//Store that computed address in T34. //store into temporary
		//(std::string symid, std::string dataReg, std::string addressReg)
	storeData(quadStatement->op3, dataReg1, "R4", "R6", true);

	//set use as ref somewhere in here.
		//op3 is the temp to be set from ST.
	st->symbolTable[quadStatement->op3]->setRefFlag("Y");
}
