...
int main()
{
	...
	b2.reserve(33);
	test_(b2.capacity() == 64);      // Assumes IType = unsigned int
	...
	// Test bit ops
	BitArray<> x{ "011010110" };
	test_(x.count() == 5);
	test_(x.any());
	test_((x << 6).to_string() == "110000000");
	test_((x >> 6).to_string() == "000000011");
	test_((x <<= 3).to_string() == "010110000"); //these take references to the original.
